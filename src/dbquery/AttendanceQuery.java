...
public class AttendanceQuery {
    ...
    public static String deriveStatus() {
        ...
        if (hour > AFTER_HOURS_HOUR || (hour == AFTER_HOURS_HOUR && minute >= 0)) return "After Hours";
        if (hour >= HALF_DAY_HOUR)   return "Half-Day";
        if (hour < LATE_HOUR || (hour == LATE_HOUR && minute == LATE_MINUTE)) return "Present";
        return "Late";
    }

    public static String peekStatus() { return deriveStatus(); }
    ...
    public static int clockOut(int empId) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (isOT) { pst.setInt(1, minutesPast5 / 60); pst.setInt(2, attendanceId); }
            else      { pst.setInt(1, attendanceId); }
            pst.executeUpdate();
        } catch (SQLException e) { e.printStackTrace(); return -1; }

        if (isOT) creditOT(empId, minutesPast5 / 60);
        return attendanceId;
    }
    ...
    private static void creditOT(int empId, int otHours) {
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pst = conn.prepareStatement(sql)) {
            pst.setInt(1, otHours); pst.setInt(2, empId);
            pst.executeUpdate();
        } catch (SQLException e) { e.printStackTrace(); }
    }

    public static boolean hasOpenEntry(int empId) { return getOpenEntryId(empId) >= 0; }
    ...
    public static void autoTimeoutForgottenEntries() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (fixed > 0) System.out.println("[AttendanceQuery] Auto-timed-out " + fixed + " forgotten entries.");
        } catch (SQLException e) { e.printStackTrace(); }
    }
    ...
    public static List<AttendanceRecord> getAttendanceReport(
        ...
        if (dateFrom  != null) sql.append("AND a.date_logged >= ? ");
        if (dateTo    != null) sql.append("AND a.date_logged <= ? ");
        if (deptFilter != null) sql.append("AND e.department = ? ");
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            int idx = 1;
            if (dateFrom   != null) pst.setString(idx++, dateFrom);
            if (dateTo     != null) pst.setString(idx++, dateTo);
            if (deptFilter != null) pst.setString(idx++, deptFilter);
